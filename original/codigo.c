...
void tabela_frequencia( const char* nome_arquivo,ulli frequencia[TAM] /* alocacao por "calloc"*/){
    //Passo 1:
...
No* remover_no_inicio(Lista* list) {
    ...
    No* aux = list->inicio; // Aponta para o primeiro nó
    list->inicio = aux->prox; // O próximo nó se torna o primeiro
    aux->prox = NULL; // Remove a ligação do nó removido com a lista
    list->tam--; // Atualiza o tamanho da lista
...
void gerar_tabela_codigos(No* raiz, char* codigo_atual, char** tabela_codigos) {
    // Achamos um nó folha
    if (raiz->dir == NULL && raiz->esq == NULL) {
        // Copia o código que formamos indo para a esquerda ou para a direita e armazena na tabela de códigos
        tabela_codigos[*(unsigned char*)(raiz->byte)] = strdup(codigo_atual);
        return;
    }

    // Vamos percorrer tanto para a esquerda quanto para a direita
    // Percorrendo para a esquerda
    int len = strlen(codigo_atual);

    // Cria uma string para a esquerda, com espaço para o novo caractere '0'
    char* aux_esquerda = (char*)malloc(len + 2);  // +2 para '0' e '\0'
    strcpy(aux_esquerda, codigo_atual);
    strcat(aux_esquerda, "0");

    // Chama recursivamente para a subárvore à esquerda
    gerar_tabela_codigos(raiz->esq, aux_esquerda, tabela_codigos);
    
    // Libera a memória alocada para o lado esquerdo
    free(aux_esquerda);

    // Percorrendo para a direita
    char* aux_direita = (char*)malloc(len + 2);  // +2 para '1' e '\0'
    strcpy(aux_direita, codigo_atual);
    strcat(aux_direita, "1");
    ...
    // Libera a memória alocada para o lado direito
    free(aux_direita);
}
*/
